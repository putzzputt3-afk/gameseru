...
async function resetProtected(){
  ...
  document.getElementById("result").innerHTML = `
    <div class='loading-box'>
      ...
      <div id='loadText' class='loading-text'>🔓 Mengambil data dari GitHub... 0%</div>
    </div>
  `;
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    txt.textContent = `🔓 Mengambil data dari GitHub... ${steps[i]}%`;
    await new Promise(r => setTimeout(r, 500));
  }
  txt.textContent = "✅ Selesai memuat data!";
  ...
  try {
    ...
    if (attempt.trim() === data.reset_password) {
      ...
      tampilkanDaftarNilai();
      document.getElementById("result").innerHTML = "🧹 Semua data nilai berhasil dihapus!";
      alert("✅ Berhasil mengambil DATABASE token");
    } else {
      alert("❌ Gagal mengambil DATABASE token");
      document.getElementById("result").innerHTML = "🔒 Akses ditolak.";
    }
  } catch {
